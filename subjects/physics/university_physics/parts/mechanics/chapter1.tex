...
\begin{itemize}
	...
	\item {Unit Prefixes such as kilo-, centi-, milli-, micro-, etc.} 
	...
	\item {Significant figures and when to round.}
\end{itemize}
...
\subsection{SI Units}
These are the basics, more units defined later are combinations of these. There are some that are not combinations, such as electric current, but those will be defined later.

\begin{itemize}
	\item Length is the meter (m)
	\item Mass is the kilogram (kg)
	\item Time is the second (s)
\end{itemize}


\subsection{Unit Prefixes}
Here are some common prefixes used in SI units. These are used to make very large or very small numbers easier to read and write.
...
When you measure your tools might have uncertainty, also known as error. This happens because the device 
used to measure has a limited amount of precision. For example a ruler might only go to centimeter so any 
value you measure with it is only accurate to the nearest centimeter.

\begin{itemize}
	\item Precision is how reliable your measurements are.
	\item Accuracy is how close your measurements are to the actual value.
\end{itemize}

\bigskip
With this in mind, when you do calculations you need to round your results to the correct number of significant figures. Here are the basic rules,

\begin{itemize}
	\item When multiplying or dividing, round to the least number of significant figures in any of the numbers used.
	\item When adding or subtracting, round to the least number of decimal places in any of the numbers used.
\end{itemize}
